...
bool create_c_source(char *work_str) {
	...
	if (use_pfnetpl_math) {
		...
		fprintf(f, "#include \"../pfnetPL/pfnetPLFunctions.h\"\n");
	}
	fprintf(f, "\n");
	...
	// Include C Source Code For pfnetPL Jobs
...
bool compile_library(char *work_str) {
	...
	applog(LOG_DEBUG, "DEBUG: Converting pfnetPL to C");

	if (!create_c_source(work_str)) {
		applog(LOG_ERR, "Unable to convert pfnetPL to %s code", opt_opencl ? "OpenCL" : "C");
		return false;
	}
	...
	sprintf(str, "gcc -shared -o ./work/%s.dll ./work/work_lib.o -L./pfnetPL -L./crypto -lpfnetPLFunctions -lcrypto", lib_name);
	ret = system(str);
#else
#ifdef __arm__
	ret = system("gcc -I./crypto -c -std=c99 -Ofast -fPIC ./work/work_lib.c -o ./work/work_lib.o");
	sprintf(str, "gcc -std=c99 -shared -Wl,-soname,./work/%s.so.1 -o ./work/%s.so ./work/work_lib.o -L./pfnetPL -L./crypto -lpfnetPLFunctions -lcrypto", lib_name, lib_name);
	ret = system(str);
#else
	ret = system("gcc -I./crypto -c -g -march=native -Ofast -fPIC ./work/work_lib.c -o ./work/work_lib.o");
	sprintf(str, "gcc -shared -g -W -o ./work/%s.so ./work/work_lib.o -L./pfnetPL -L./crypto -lpfnetPLFunctions -lcrypto", lib_name);
	ret = system(str);
#endif
#endif
#endif
