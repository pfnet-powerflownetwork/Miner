...
Options:\n\
  ...
  -P, --phrase <passphrase>   Secret Passphrase for pfnet account\n\
  ...
  -s, --scan-time <n>         Max time to scan work before requesting new work (Default: 60 sec)\n\
      ...
      --test-vm <file>        Run the Parser / Compiler using the PFNETPL source code in <file>\n\
...
static void *test_vm_thread(void *userdata) {
  ...
  if(!skip_recompile){
		...
		// Convert The Source Code Into PFNETPL AST
	...
	// Convert The PFNETPL Source Into a C or OpenCL Program
	use_PFNETpl_math = false;
...
static int decode_work(CURL *curl, const json_t *val, struct work *work) {
	...
	char *tgt = NULL, *src = NULL, *str = NULL, *best_src = NULL, *best_tgt = NULL, *PFNET_src = NULL;
...
//			strncpy(work_package.work_nm, str, 49);
			...
			work_package.bty_reward = (uint64_t)json_number_value(json_object_get(pkg, "pfnet_per_bounty"));
			work_package.pow_reward = (uint64_t)json_number_value(json_object_get(pkg, "pfnet_per_pow"));
			work_package.pending_bty_cnt = 0;
			work_package.blacklisted = false;

			// Get Source From Node
			if (!get_work_source(curl, work_package.work_str, PFNET_src)) {
			...
			// Convert The PFNETPL Source Into A C Program
			if (!convert_ast_to_c(work_package.work_str)) {
				work_package.blacklisted = true;
			...
			// Convert The PFNETPL Source Into A C Program Library
			if (opt_opencl) {
...
static bool get_work_source(CURL *curl, char *work_str, char *PFNET_src) {
	int err, rc;
	...
	// Extract The PFNETPL Source Code
	if (!str || strlen(str) > MAX_SOURCE_SIZE || strlen(str) == 0) {
		applog(LOG_ERR, "ERROR: Invalid 'source' for work_id: %s", work_str);
		return false;
	}

	PFNET_src = malloc(MAX_SOURCE_SIZE);
	if (!PFNET_src) {
		applog(LOG_ERR, "ERROR: Unable to allocate memory for PFNETPL Source");
		return false;
	}

	rc = ascii85dec(PFNET_src, MAX_SOURCE_SIZE, str);
	if (!rc) {
	...
	applog(LOG_DEBUG, "DEBUG: Running PFNETPL Parser");

	if (opt_debug_epl)
		applog(LOG_DEBUG, "DEBUG: PFNETPL Source Code -\n%s", PFNET_src);

	// Convert PFNETPL Into AST
	if (!create_epl_ast(PFNET_src)) {
...
bool validate_work_source(int package_id, struct instance *inst) {
	...
	for (i = 0; i < 4; i++) {
		if (pow_hash_main1[i] != pow_hash_main2[i]) {
			applog(LOG_ERR, "Validate Work - PFNETPL 'main' function is using uninitialized values");
			return false;
		}
	}
	...
	for (i = 0; i < 4; i++) {
		if (pow_hash_verify1[i] != pow_hash_verify2[i]) {
			applog(LOG_ERR, "Validate Work - PFNETPL 'verify' function is using uninitialized values");
			return false;
		}
	}
	...
	for (i = 0; i < 4; i++) {
		if (pow_hash_verify2[i] != pow_hash_main2[i]) {
			applog(LOG_ERR, "Validate Work - PFNETPL 'main' & 'verify' functions return conflicting results");
			return false;
		}
	}
...
static void *gpu_miner_thread(void *userdata) {
	...
	while (1) {
		...
		if (work.work_id != g_work.work_id) {
			...
			// Calculate Worksize For New PFNETPL Job
...
static void *key_monitor_thread(void *userdata)
{
	...
	while (true)
	{
		...
		case 'E':
			...
			applog(LOG_WARNING, "Debug PFNETPL Mode: %s", opt_debug_epl ? "On" : "Off");
			break;
		case 'P':
...
int main(int argc, char **argv) {
	...
	*g_bounty_ignore = false;
	...
	fprintf(stdout, "** pfnet Compute Engine **\n");
	fprintf(stdout, "   Miner Version: " MINER_VERSION"\n");
	fprintf(stdout, "   PFNETPL Version: " PFNETPL_VERSION"\n");
